...
- [IN-18 L269] HttpPing event (cat=ping from Rust backend) sets apiLatencyMs; ApiFetch events also update apiLatencyMs from their round-trip duration, but HttpPing overrides with a cleaner dedicated measurement when available.

## Session Resume

- [SR-02 L7] Token/cost counters show only NEW conversation usage (inspector starts accumulating from connection time)
- [SR-03 L166] ResumePicker uses persisted metadata.nodeSummary as the dead-session summary; nodeSummary is captured from the first top-level user input in tapMetadataAccumulator.
