...
impl ToolCallEmission {
    fn from_output_item(item: &Value) -> Self {
        let call_id = item
            .get("call_id")
            .and_then(|v| v.as_str())
            .unwrap_or("")
...
pub fn shape_function_calls(function_calls: &[Value]) -> ShapedToolCalls {
    let calls: Vec<ToolCallEmission> = function_calls
        .iter()
        .map(ToolCallEmission::from_output_item)
        .collect();
    ...
    let summary = ToolCallShapeSummary {
        ...
        shaping_applied: suppressed_tool_call_count > 0,
        shaping_policy: if suppressed_tool_call_count > 0 {
            Some(WORKTREE_AGENT_SHAPING_POLICY.to_string())
        } else {
            None
        },
    };
...
mod tests {
    ...
    fn test_shape_function_calls_suppresses_only_extra_worktree_agents() {
        ...
        let shaped = shape_function_calls(&calls);
